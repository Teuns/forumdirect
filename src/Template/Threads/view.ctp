    ...
    <div class="box_stuff">
        <?php if($currPage == 0): ?>
            <div class="comment initial">
                ...
                <p class="comment-time-stamp"><?= $thread->created->i18nFormat('MMM dd, yyyy h:mm:ss a') ?> <?php if($thread->modified): ?> modified at <?= $thread->modified->i18nFormat('MMM dd, yyyy h:mm:ss a') ?> <?php endif; ?> <?php if($loggedIn && $userId == $thread->user_id || $loggedIn && $role == 'admin' || $loggedIn && $role == 'mod'): ?> <?= $this->Html->link('Edit', ['action' => 'edit', $thread->id]) ?> <?php endif; ?></p>
            </div>
        <?php endif; ?>
        ...
        <?php foreach($posts as $post): ?>
            <div class="comment initial">
                ...
                <p class="comment-time-stamp"><?= $post->created->i18nFormat('MMM dd, yyyy h:mm:ss a') ?> <?php if($post->modified): ?> modified at <?= $post->modified->i18nFormat('MMM dd, yyyy h:mm:ss a') ?> <?php endif; ?> <?php if($loggedIn && $userId == $post->user_id || $loggedIn && $role == 'admin' || $loggedIn && $role == 'mod'): ?> <?= $this->Html->link('Edit', ['controller' => 'Posts', 'action' => 'edit', $post->id]) ?> <?php endif; ?></p>
            </div>
        <?php endforeach; ?>
    </div>
</div>
